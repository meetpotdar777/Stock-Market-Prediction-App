...
def prepare_data(df, lag_days, predict_future_days):
    """
    Replaces the previous prepare_data function.
...
def train_and_predict(X_scaled, y, model_choice):
    """
    Replaces the previous train_and_predict function.
...
def visualize_predictions(y_train, y_test, y_train_pred, y_test_pred, predict_future_days,
    ...
    """
    Replaces the previous visualize_predictions function.
...
if __name__ == "__main__":
    ...
    # Variables to store historical data for visualization if model is loaded
    processed_df_for_viz = None
    ...
    if os.path.exists(MODEL_FILE) and os.path.exists(SCALER_FILE) and os.path.exists(FEATURE_COLUMNS_FILE):
        ...
        try:
            ...
            # If loaded, we still need to re-generate the data to get y_train, y_test, etc. for visualization
            print("Re-generating data for visualization (using historical range)...")
            ...
            if stock_df_hist is None:
                ...
                if model_trained_with_economic_features:
                ...
                X_scaled_hist, y_hist, processed_df_for_viz, _, _ = prepare_data(df_with_all_features_hist, N_LAG_DAYS, PREDICT_N_DAYS_FUTURE)

                if not X_scaled_hist.empty and not y_hist.empty:
                    X_train_hist, X_test_hist, y_train_for_viz, y_test_for_viz = train_test_split(X_scaled_hist, y_hist, test_size=0.2, random_state=42, shuffle=False)
                    y_train_pred_for_viz = trained_model.predict(X_train_hist)
                    y_test_pred_for_viz = trained_model.predict(X_test_hist)
                    # Capture the last known close price for the visualization
            ...
            print(f"Error loading saved files or preparing historical data for visualization: {e}. Proceeding with fresh training if needed.")
            trained_model = None # Reset to trigger retraining if load failed
            processed_df_for_viz = None # Clear visualization data
    else:
    ...
    if trained_model is None: # This block is executed if model was not loaded (or load failed)
        # 1. Fetch Stock Data
        ...
        if not X_scaled.empty and not y.empty:
            ...
            # Assign these for potential visualization later
            processed_df_for_viz = processed_df
            y_train_for_viz = y_train
            y_test_for_viz = y_test
            y_train_pred_for_viz = y_train_pred
            y_test_pred_for_viz = y_test_pred
    ...
    # Debug prints for variable states right before the prediction block guard
    ...
    if trained_model is not None and fitted_scaler is not None and feature_columns_loaded:
        ...
        try:
            ...
            # Use the global END_DATE for fetching data for prediction
            ...
            if any(col.startswith(('Federal_Funds_Rate', 'CPI', 'Unemployment_Rate')) for col in feature_columns_loaded):
                if not FRED_API_KEY or FRED_API_KEY == "YOUR_FRED_API_KEY_HERE":
                    ...
                    # Use the global END_DATE for fetching economic data as well
