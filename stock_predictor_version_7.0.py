...
def visualize_predictions(original_df, y_train, y_test, y_train_pred, y_test_pred, predict_future_days):
    ...
    # Combine actual training and testing target values
    ...
    # Ensure prediction dates align with the actual dates
    # Since y_train and y_test maintain their original indices, we can use them
    prediction_dates_actuals = all_actuals.index # Dates corresponding to the actual values

    plt.plot(prediction_dates_actuals,
             all_actuals,
             label='Actual Future Close Price', color='blue', linewidth=2)

    plt.plot(prediction_dates_actuals, # Use the same dates for predictions for direct comparison
             all_predictions,
