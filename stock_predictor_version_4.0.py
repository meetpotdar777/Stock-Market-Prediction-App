...
def add_features(df, economic_df, lag_days):
    ...
    if economic_df is not None and not economic_df.empty:
        # Align economic data with stock data index
        ...
        # Forward fill any NaNs introduced by merging (economic data is often lower frequency)
        df_with_features.ffill(inplace=True)
    else:
    ...
    df_with_features['Stoch_D'] = ta.momentum.stoch_signal(high=high_series, low=low_series, close=close_series, window=3, fillna=False)
    ...
    df_with_features['PVT'] = df_with_features['PVT'].cumsum().fillna(0)


    # Temporal Features (from original request, now after merging economic data to ensure index alignment)
    ...
    # --- Generate Lagged Features for all relevant (non-target, non-metadata) columns ---
    # Identify columns that are candidates for lagging
    # Exclude original OHLCV, Adj_Close, and target-like columns if they exist at this stage
    ...
    # Add economic features to the list of candidates if they were merged
    economic_cols_present = [col for col in economic_df.columns if col in df_with_features.columns] if economic_df is not None else []
    features_to_lag_candidates.extend(economic_cols_present)

    # Filter out columns that might not exist in the current df_with_features
    # This is important if some indicators couldn't be calculated or economic data was not merged.
    features_to_lag = [f for f in features_to_lag_candidates if f in df_with_features.columns]

    print(f"Generating lag features for {len(features_to_lag)} features with {lag_days} lags...")
    for feature in features_to_lag:
        for i in range(1, lag_days + 1):
            df_with_features[f'{feature}_Lag_{i}'] = df_with_features[feature].shift(i)
    
    print("Lag features generated.")
    
    # Also add rate of change for economic indicators (as per original code intent)
    for col in economic_cols_present:
        if f'{col}_ROC_1' not in df_with_features.columns:
            df_with_features[f'{col}_ROC_1'] = df_with_features[col].pct_change()
...
if __name__ == "__main__":
    ...
    if trained_model is not None and fitted_scaler is not None and feature_columns_loaded:
        ...
        try:
            ...
            # This check ensures consistency with the loaded model's training features
            if any(col.startswith(('Federal_Funds_Rate', 'CPI', 'Unemployment_Rate')) for col in feature_columns_loaded):
                if not FRED_API_KEY or FRED_API_KEY == "YOUR_FRED_API_KEY_HERE":
                    ...
                    latest_economic_df = None # Still proceed as None, but with a strong warning.
                else:
                    latest_economic_df = fetch_economic_data(START_DATE, END_DATE, FRED_API_KEY)
            ...
            else: # Model was trained WITHOUT economic features
                latest_economic_df = None # Do not fetch economic data if the model wasn't trained with it

            # Determine the maximum historical window required for indicators and lags.
            # This includes the technical indicators and the rolling volatility.
            # The +1 is for the current day's features needed to predict the next day.
            # Add N_LAG_DAYS for the lagged features.
            # Adding a buffer for robustness.
            # The largest window for TA is 26 (MACD), largest for lags is N_LAG_DAYS.
            # So, to get features for today, we need (Max_TA_Window + N_LAG_DAYS) days of data.
            # Plus a few extra for robustness if data is sparse or holidays.
            required_raw_data_points = MAX_INDICATOR_CALCULATION_WINDOW + N_LAG_DAYS + 10 # Increased buffer for safety
            ...
            # Ensure the economic data passed here is also sliced/aligned correctly if it has future dates
            # For prediction, we need economic data up to the last known stock date.
            if latest_economic_df is not None:
                latest_economic_df_aligned = latest_economic_df.loc[data_for_prediction_features_raw.index.min():data_for_prediction_features_raw.index.max()].copy()
            else:
                latest_economic_df_aligned = None

            engineered_features_for_pred = add_features(data_for_prediction_features_raw, latest_economic_df_aligned, N_LAG_DAYS)
            ...
            if not engineered_features_for_pred.empty:
                ...
                # Ensure the features are in the same order as trained_model expects
                ...
                # This is crucial for consistency between training and prediction, especially if economic data
                # was or wasn't part of the training set.
                ...
                if extra_cols_in_pred:
                    ...
                    engineered_features_for_pred = engineered_features_for_pred[feature_columns_loaded]

                # Select features and ensure order
                next_day_features_df = engineered_features_for_pred.iloc[-1][feature_columns_loaded].to_frame().T
