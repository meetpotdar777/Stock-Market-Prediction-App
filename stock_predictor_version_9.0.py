...
# Option to set a custom end date.
# Set to 'YYYY-MM-DD' string (e.g., "2024-01-01") if you want a specific end date.
# Set to None if you want to use today's date as the end date.
CUSTOM_END_DATE = None # <--- SET YOUR CUSTOM END DATE HERE (e.g., "2025-01-01")

# Define the start and end dates for historical data
START_DATE = "2025-01-01"
END_DATE = CUSTOM_END_DATE if CUSTOM_END_DATE else datetime.date.today().strftime("%Y-%m-%d")
...
def add_features(df, economic_df, lag_days):
    ...
    Optimized to ensure features are added efficiently.
    ...
    # Dictionary to collect all new features before concatenation
    features_to_add = {}

    # --- Add primary features to features_to_add dictionary ---
    features_to_add['Daily_Return'] = close_series.pct_change(fill_method=None) 
    features_to_add['SMA_10'] = ta.trend.sma_indicator(close=close_series, window=10, fillna=False)
    features_to_add['SMA_20'] = ta.trend.sma_indicator(close=close_series, window=20, fillna=False)
    features_to_add['RSI'] = ta.momentum.rsi(close=close_series, window=14, fillna=False)
    features_to_add['MACD'] = ta.trend.macd(close=close_series, fillna=False)
    features_to_add['BB_High'] = ta.volatility.bollinger_hband(close=close_series, fillna=False)
    features_to_add['BB_Low'] = ta.volatility.bollinger_lband(close=close_series, fillna=False)
    features_to_add['BB_Width'] = ta.volatility.bollinger_wband(close=close_series, window=20, fillna=False)
    features_to_add['BB_Percent_B'] = ta.volatility.bollinger_pband(close=close_series, window=20, fillna=False)
    features_to_add['ATR'] = ta.volatility.average_true_range(high=high_series, low=low_series, close=close_series, window=14, fillna=False)
    features_to_add['OBV'] = ta.volume.on_balance_volume(close=close_series, volume=volume_series, fillna=False)
    features_to_add['Stoch_K'] = ta.momentum.stoch(high=high_series, low=low_series, close=close_series, window=14, fillna=False)
    features_to_add['Stoch_D'] = ta.momentum.stoch_signal(high=high_series, low=low_series, close=close_series, window=3, fillna=False)
    features_to_add['ROC'] = ta.momentum.roc(close=close_series, window=12, fillna=False)
    features_to_add['CMF'] = ta.volume.chaikin_money_flow(high=high_series, low=low_series, close=close_series, volume=volume_series, window=20, fillna=False)
    
    # Rolling Volatility (Standard Deviation of Daily Returns)
    # Access Daily_Return from features_to_add dictionary
    features_to_add['Rolling_Vol_20'] = features_to_add['Daily_Return'].rolling(window=20).std()

    # Price-Volume Trend (PVT)
    pvt_series = (close_series.diff() / close_series.shift(1)) * volume_series
    features_to_add['PVT'] = pvt_series.cumsum().fillna(0)

    # Temporal Features
    features_to_add['Day_Of_Week'] = pd.Series(df_temp.index.dayofweek, index=df_temp.index)
    features_to_add['Day_Of_Month'] = pd.Series(df_temp.index.day, index=df_temp.index)
    features_to_add['Week_Of_Year'] = pd.Series(df_temp.index.isocalendar().week.astype(int), index=df_temp.index)
    features_to_add['Month_Of_Year'] = pd.Series(df_temp.index.month, index=df_temp.index)
    features_to_add['Quarter_Of_Year'] = pd.Series(df_temp.index.quarter, index=df_temp.index)

    # Add rate of change for economic indicators (if applicable)
    for col in economic_cols_present:
        features_to_add[f'{col}_ROC_1'] = df_temp[col].pct_change(fill_method=None) 

    # Concatenate all new features to the main DataFrame at once
    df_temp = pd.concat([df_temp, pd.DataFrame(features_to_add, index=df_temp.index)], axis=1)
    ...
    features_to_lag_candidates = list(features_to_add.keys()) # All features just created
    features_to_lag_candidates.extend(['Open', 'High', 'Low', 'Close', 'Volume']) # Original columns
    features_to_lag_candidates.extend(economic_cols_present) # Economic columns

    # Filter out duplicates and ensure they exist in df_temp before lagging
    features_to_lag = [f for f in set(features_to_lag_candidates) if f in df_temp.columns and f not in ['Adj_Close', 'Target']]
...
def visualize_predictions(y_train, y_test, y_train_pred, y_test_pred, predict_future_days,
                          future_prediction_date=None, future_predicted_price=None,
                          last_historical_close=None, last_historical_close_date=None):
    """
    Visualizes the actual and predicted stock prices, including a future prediction if provided.
    ...
    Args:
        ...
        future_prediction_date (datetime.datetime or None): The date for the single future prediction.
        future_predicted_price (float or None): The value of the single future prediction.
        last_historical_close (float or None): The very last known actual close price from the dataset.
        last_historical_close_date (datetime.datetime or None): The date of the last known actual close price.
    """
    plt.figure(figsize=(14, 7))

    # Combine actual training and testing target values (these are already future prices for their dates)
    all_actuals = pd.concat([y_train, y_test])
    
    # Combine predicted training and testing values
    all_predictions_historical = np.concatenate((y_train_pred, y_test_pred))
    
    # Dates corresponding to the actual values (which are also the dates for historical predictions)
    historical_prediction_dates = all_actuals.index

    # Plot actual future close prices
    plt.plot(historical_prediction_dates,
             all_actuals,
             label='Actual Future Close Price', color='blue', linewidth=2)

    # Plot historical model predictions
    plt.plot(historical_prediction_dates,
             all_predictions_historical,
             label='Model Historical Predictions', color='red', linestyle='--', alpha=0.7)

    # Plot the last known historical close price if provided
    if last_historical_close is not None and last_historical_close_date is not None:
        plt.plot(last_historical_close_date, last_historical_close, 
                 marker='X', markersize=10, color='purple', label='Last Known Actual Close')
        # Draw a dotted line from the last historical actual price to the corresponding last historical prediction date's actual future price
        # This connects the end of the historical 'Close' series to the beginning of the 'Actual Future Close Price' series.
        # This is primarily for visual context if the gap between `df['Close']` and `y_train` dates is large due to `dropna()`
        
        # Find the first date in `all_actuals` (which is y_train's first date)
        first_future_actual_date = historical_prediction_dates[0]
        first_future_actual_price = all_actuals.iloc[0]

        # Only draw this line if the last_historical_close_date is directly preceding or close to first_future_actual_date
        # and if it makes sense visually (i.e., not a huge time gap)
        if (first_future_actual_date - last_historical_close_date).days < 30: # Within approx 1 month
            plt.plot([last_historical_close_date, first_future_actual_date],
                     [last_historical_close, first_future_actual_price],
                     color='gray', linestyle=':', alpha=0.5, label='_Hidden connection to future actuals')


    # Plot the single future prediction if available
    if future_prediction_date and future_predicted_price is not None:
        # Get the last historical prediction point to draw a line from
        # We need the last date of the *historical predictions* to smoothly extend the line
        if len(historical_prediction_dates) > 0 and len(all_predictions_historical) > 0:
            last_historical_pred_date = historical_prediction_dates[-1]
            last_historical_pred_value = all_predictions_historical[-1]
        else: # Fallback if historical predictions are empty (unlikely with enough data)
            last_historical_pred_date = last_historical_close_date if last_historical_close_date else pd.Timestamp(END_DATE)
            last_historical_pred_value = last_historical_close if last_historical_close is not None else all_actuals.iloc[-1] if not all_actuals.empty else 0


        plt.plot([last_historical_pred_date, future_prediction_date],
                 [last_historical_pred_value, future_predicted_price],
                 marker='o', markersize=8, color='green', linestyle='-', linewidth=2,
                 label=f'Future Prediction ({predict_future_days} Day)')
        
        # Add annotation for the future predicted price
        plt.annotate(f'${future_predicted_price:.2f}', 
    ...
    plt.gcf().autofmt_xdate() # Format x-axis labels for dates
    plt.show()
...
if __name__ == "__main__":
    ...
    # Variables to store historical data for visualization if model is loaded or trained
    y_train_for_viz = None
    y_test_for_viz = None
    y_train_pred_for_viz = None
    y_test_pred_for_viz = None
    last_known_close_price = None
    last_known_close_date = None

    # Variables for future prediction
    next_future_price_pred = None
    ...
    # Attempt to load existing model, scaler, and feature columns
    ...
    if os.path.exists(MODEL_FILE) and os.path.exists(SCALER_FILE) and os.path.exists(FEATURE_COLUMNS_FILE):
        ...
        try:
            ...
            print(f"Error loading saved files: {e}. Will attempt to train a new model.")
            trained_model = None # Ensure retraining if load failed
            fitted_scaler = None
            feature_columns_loaded = []
            model_loaded_successfully = False
    else:
        print("No existing model, scaler, or feature columns found.")
    
    # If model was not loaded, proceed with training
    if not model_loaded_successfully:
        print("Proceeding with data fetching and training a new model.")
        # 1. Fetch Stock Data
        ...
        # 5. Train and Predict (for historical visualization)
        if not X_scaled.empty and not y.empty:
            trained_model, y_train_pred_for_viz, y_test_pred_for_viz, X_train, X_test, y_train_for_viz, y_test_for_viz = train_and_predict(X_scaled, y, MODEL_CHOICE)
            ...
            feature_columns_loaded = feature_columns_for_saving # Ensure this is set for future prediction
            last_known_close_price = stock_df['Close'].iloc[-1]
            last_known_close_date = stock_df.index[-1]
        else:
            print("Data preparation resulted in empty features or target. Cannot train model.")
            exit()
    else: # Model was loaded successfully, now prepare historical data for visualization using the loaded model
        print("Preparing historical data for visualization using loaded model...")
        stock_df_hist = fetch_stock_data(STOCK_TICKER, START_DATE, END_DATE)
        if stock_df_hist is None:
            print("Failed to fetch historical stock data for visualization. Skipping historical plot.")
        else:
            model_trained_with_economic_features = any(
                col.startswith(('Federal_Funds_Rate', 'CPI', 'Unemployment_Rate')) for col in feature_columns_loaded
            )
            if model_trained_with_economic_features and (not FRED_API_KEY or FRED_API_KEY == "YOUR_FRED_API_KEY_HERE"):
                print("Skipping economic data fetch for historical visualization due to missing FRED API Key.")
                economic_df_hist = None
            else:
                economic_df_hist = fetch_economic_data(START_DATE, END_DATE, FRED_API_KEY)
            
            df_with_all_features_hist = add_features(stock_df_hist, economic_df_hist, N_LAG_DAYS)
            
            # Use the loaded scaler and feature columns for preparing historical data for prediction
            # Need to re-create X_scaled_hist and y_hist in the same way as training
            df_with_features_viz = df_with_all_features_hist.copy()
            df_with_features_viz['Target'] = df_with_features_viz['Close'].shift(-PREDICT_N_DAYS_FUTURE)
            df_with_features_viz.dropna(inplace=True)

            if not df_with_features_viz.empty:
                features_to_exclude_viz = ['Target', 'Adj_Close', 'Close']
                numeric_features_list_viz = [col for col in feature_columns_loaded if col in df_with_features_viz.columns]
                X_hist_viz = df_with_features_viz[numeric_features_list_viz]
                y_hist_viz = df_with_features_viz['Target']

                X_scaled_hist_viz = pd.DataFrame(fitted_scaler.transform(X_hist_viz), columns=X_hist_viz.columns, index=X_hist_viz.index)
                
                X_train_hist_viz, X_test_hist_viz, y_train_for_viz, y_test_for_viz = train_test_split(X_scaled_hist_viz, y_hist_viz, test_size=0.2, random_state=42, shuffle=False)
                
                y_train_pred_for_viz = trained_model.predict(X_train_hist_viz)
                y_test_pred_for_viz = trained_model.predict(X_test_hist_viz)
                
                last_known_close_price = stock_df_hist['Close'].iloc[-1]
                last_known_close_date = stock_df_hist.index[-1]
            else:
                print("Historical data preparation for visualization resulted in empty data. Skipping historical plot.")


    # --- Predict the next 'PREDICT_N_DAYS_FUTURE' days (works with loaded or newly trained model) ---
    print(f"DEBUG (Main): State before prediction block:")
    print(f"DEBUG (Main): trained_model is None: {trained_model is None}")
    print(f"DEBUG (Main): fitted_scaler is None: {fitted_scaler is None}")
    print(f"DEBUG (Main): feature_columns_loaded is empty: {not feature_columns_loaded}")
    ...
    if trained_model is not None and fitted_scaler is not None and feature_columns_loaded:
        ...
        try:
            ...
            if latest_stock_df is None:
                ...
                # Don't exit, allow visualization of historical data if available
            else:
                model_trained_with_economic_features = any(
                    col.startswith(('Federal_Funds_Rate', 'CPI', 'Unemployment_Rate')) for col in feature_columns_loaded
                )
                ...
                if not engineered_features_for_pred.empty:
                    ...
                    if missing_cols_in_pred:
                        ...
                        # exit("Exiting due to feature mismatch for prediction.") # Do not exit, just skip prediction
                    else:
                        ...
                        last_known_date_for_pred = latest_stock_df.index[-1]
                        prediction_date = last_known_date_for_pred + pd.Timedelta(days=PREDICT_N_DAYS_FUTURE)
    ...
    # --- Final Visualization (if historical data and predictions are available) ---
    if y_train_for_viz is not None and y_test_for_viz is not None and \
       y_train_pred_for_viz is not None and y_test_pred_for_viz is not None:
        visualize_predictions(
            y_train=y_train_for_viz,
            y_test=y_test_for_viz,
            y_train_pred=y_train_pred_for_viz,
            y_test_pred=y_test_pred_for_viz,
            predict_future_days=PREDICT_N_DAYS_FUTURE,
            future_prediction_date=prediction_date, # Pass the single future prediction details
            future_predicted_price=next_future_price_pred[0] if next_future_price_pred is not None else None,
            last_historical_close=last_known_close_price,
            last_historical_close_date=last_known_close_date
        )
    else:
        print("\nSkipping historical prediction visualization due to missing data.")
